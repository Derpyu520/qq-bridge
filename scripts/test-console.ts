...
function readConsoleToken(): string {
  try {
    const cfg: any = JSON.parse(fs.readFileSync(path.join(ROOT, 'config.json'), 'utf8'));
...
const api = async (path: string, method?: string, body?: any): Promise<{ status: number; body: any }> => {
  const headers: Record<string, string> = { ...authHeaders, ...(body ? { 'content-type': 'application/json' } : {}) };
