...
try {
  ...
  const groups: any = await bot.raw('get_group_list', {});
  const list = Array.isArray(groups) ? groups : groups?.data ?? [];
  console.log(`共 ${list.length} 个群：`, list.map((g: any) => `${g.group_id}(${g.group_name})`).join('、'));

  const target = list.find((g: any) => String(g.group_name ?? '').includes(keyword));
  if (!target) {
    console.error(`❌ 未找到名称包含「${keyword}」的群`);
    process.exit(1);
  }

  const result: any = await bot.sendGroupMessage(target.group_id, text(message));
