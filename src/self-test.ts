...
// 用法：bun src/self-test.ts [baseUrl]
...
async function main(): Promise<void> {
  ...
  const done = new Promise<{ reason: { kind: string }; text: string }>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('等待回复超时（60s）')), 60_000);
    void (async () => {
      const abort = new AbortController();
      for await (const envelope of api.events.mux({}, abort.signal, () => resolveOpened())) {
...
main().catch((error) => {
  console.error('❌ 自测失败:', error instanceof Error ? error.message : error);
  process.exit(1);
});
