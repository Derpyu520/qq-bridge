...
function getAccess(): AccessConfig {
  const c = getConfig();
  return {
    allowGroups: (c.allow?.groups ?? []).map(String) as string[],
    allowPrivate: (c.allow?.private ?? []).map(String) as string[],
    denyGroups: (c.deny?.groups ?? []).map(String) as string[],
    denyPrivate: (c.deny?.private ?? []).map(String) as string[],
    allowAllWhenEmpty: c.allowAllWhenEmpty === true,
  };
}

function getOneBotConfig(): { httpUrl: string; token: string } {
  const c = getConfig();
  return {
    httpUrl: String(c.snowluma?.httpUrl ?? 'http://127.0.0.1:3000').replace(/\/+$/, ''),
    token: String(c.snowluma?.accessToken ?? ''),
  };
}

// 与 bridge.allowed 保持一致：allow 列表为空时按 allowAllWhenEmpty 放行
function isAllowed(allowList: string[], denyList: string[], id: unknown, allowAllWhenEmpty: boolean): boolean {
  const s = String(id);
...
function escapeCqText(text: unknown): string {
  return String(text ?? '').replace(/\[CQ:/gi, '[CQ：');
}

// 兼容模型把单条消息序列化成 JSON 字符串的情况，例如 "\"你好\"" → "你好"。
function unquoteJsonString(value: unknown): unknown {
...
type OneBotSegment = { type: string; data: Record<string, string> };

// 构造可选的“引用/回复”消息段。
function messageSegments(message: unknown, replyToMessageId: unknown): OneBotSegment[] {
  const segments: OneBotSegment[] = [];
  const replyId =
    replyToMessageId !== undefined && replyToMessageId !== null && String(replyToMessageId).trim() !== ''
...
async function onebot(action: string, params: Record<string, unknown> = {}): Promise<any> {
  ...
  const body = (await res.json()) as { status?: string; retcode?: number; data?: any; wording?: string };
...
function readConsoleToken(): string {
...
async function agentApi(pathName: string, init: AgentApiInit = {}): Promise<any> {
  ...
  const res = await fetch(`${agentApiBase()}${pathName}`, { ...rest, headers, signal: AbortSignal.timeout(timeoutMs) });
  let body: any = null;
  try {
    body = await res.json();
  } catch {
...
async function authorizeRead(key: string, token?: string): Promise<void> {
...
server.tool(
  ...
  async () => {
    try {
      ...
      let status: any = {};
      try {
        status = await onebot('get_status');
      } catch {}
      return { content: [{ type: 'text', text: JSON.stringify({ ...login, online: status.online, good: status.good }, null, 2) }] };
    } catch (error) {
      return { content: [{ type: 'text', text: `查询失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async () => {
    try {
      const status = (await agentApi('/api/status')) as { mode?: string };
      if (status?.mode === 'reserved2') {
        return { content: [{ type: 'text', text: 'reserved2 模式下旧只读工具不可用，请使用带会话令牌的 v2 读工具' }], isError: true };
      }
    } catch (error) {
      return { content: [{ type: 'text', text: `无法确认当前模式，拒绝执行：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
    try {
      const a = getAccess();
      const data = await onebot('get_group_list');
      const list = (Array.isArray(data) ? data : data?.data ?? [])
        .filter((g: any) => isAllowed(a.allowGroups, a.denyGroups, g.group_id, a.allowAllWhenEmpty))
...
server.tool(
  ...
  async ({ groupId }) => {
    ...
    try {
      await authorizeRead(`group:${g}`);
    } catch (error) {
      return { content: [{ type: 'text', text: `拒绝读取：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
    try {
      const data = await onebot('get_group_member_list', { group_id: Number(g) });
      const list = (Array.isArray(data) ? data : data?.data ?? []).map((m: any) => ({ user_id: m.user_id, nickname: m.nickname, card: m.card }));
      return { content: [{ type: 'text', text: JSON.stringify(list, null, 2) }] };
    } catch (error) {
      return { content: [{ type: 'text', text: `查询失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
...
server.tool(
  ...
  async ({ groupId, messageSeq }) => {
    ...
    try {
      const params: Record<string, unknown> = { group_id: Number(g) };
...
server.tool(
  ...
  async ({ key, token }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `获取提示词失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, limit }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `获取未读消息失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, limit, offset }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `获取最近消息失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `获取状态失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `标记已读失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, config }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `设置唤醒配置失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ groupId, token, messages }) => {
    try {
      ...
      let finalMessages: string | string[] = messages;
      ...
      return { content: [{ type: 'text', text: `分条发送失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
  ...
  server.tool(
    ...
    async ({ key, token, targetUserId }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `拍一拍失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
...
server.tool(
  ...
  async ({ key, token, timeoutMs, minNewMessages, quietMs }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `等待失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, level, message }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `反馈失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, limit }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `获取自己消息失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, messageId }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `获取消息详情失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, limit }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `获取活跃成员失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, category, content, extra }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `记忆写入失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, category }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `记忆读取失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, category, content, target }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `记忆删除失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, category }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `记忆清空失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, q }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `查询黑话失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
...
server.tool(
  ...
  async ({ key, token, content, context }) => {
    try {
      ...
      return { content: [{ type: 'text', text: `提交黑话失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
    }
  },
);
  ...
  server.tool(
    ...
    async ({ key, messageId, token }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `获取图片失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
  ...
  server.tool(
    ...
    async ({ key, token, query, count, refresh }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `获取表情列表失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
  ...
  server.tool(
    ...
    async ({ key, token, stickerId }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `获取表情图片失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
  ...
  server.tool(
    ...
    async ({ key, token, stickerId, replyToMessageId, atUserId }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `发送表情失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
  ...
  server.tool(
    ...
    async ({ key, token, messageId, remark }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `收藏表情失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
  ...
  server.tool(
    ...
    async ({ key, token }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `获取形象图片失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
...
if (cfg.socialV2?.sticker?.enabled !== false && cfg.socialV2?.tools?.stickerNote !== false) {
  server.tool(
    ...
    async ({ key, token, stickerId, note, tags, usage }) => {
      try {
        const payload: Record<string, unknown> = { key, stickerId: String(stickerId) };
        ...
        return { content: [{ type: 'text', text: `记录表情备注失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
  ...
  server.tool(
    ...
    async ({ key, token, stickerId, remark }) => {
      try {
        ...
        return { content: [{ type: 'text', text: `修改表情备注失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
  ...
  server.tool(
    ...
    async ({ key, token, id }) => {
      try {
        ...
        const collectMedia = (msgs: unknown) => {
          if (!Array.isArray(msgs)) return;
          for (const m of msgs) {
            if (!m || typeof m !== 'object') continue;
            for (const media of Array.isArray((m as any).media) ? (m as any).media : []) {
        ...
        if (images.length) {
          try {
            ...
            const mediaImages = Array.isArray(mediaRes?.images) ? (mediaRes.images as any[]) : [];
            ...
            imageTexts.push(`[转发内图片（批量获取失败：${error instanceof Error ? error.message : String(error)}）]`);
          }
        }
        ...
        return { content: [{ type: 'text', text: `查看合并转发失败：${error instanceof Error ? error.message : String(error)}` }], isError: true };
      }
    },
  );
}
